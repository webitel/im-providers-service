...
func TestRenderInstagram(t *testing.T) {
	tests := []struct {
		name     string
		text     string
		...
		{
			name: "bold/italic markers degrade, undocumented by Messaging API",
			text: "hello world",
			entities: []sharedmodel.Entity{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := RenderInstagram(tt.text, tt.entities)
			if got != tt.want {
...
func TestParseInstagramMarkdown(t *testing.T) {
	tests := []struct {
		name string
		text string
		want string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := ParseInstagramMarkdown(tt.text)
			if got != tt.want {
				t.Errorf("got %q, want %q", got, tt.want)
			}
		})
