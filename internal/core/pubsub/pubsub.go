...
func New(log *slog.Logger, address string) (*Manager, error) {
	m := &Manager{
		address:         address,
		log:             log,
	...
	for {
		...
		for notifyClose != nil || chanNotifyClose != nil {
			...
			case err := <-chanNotifyClose:
				m.log.Error("closed rabbitmq/pubsub connection.. attempting to reconnect", slog.Any("err", err))
			...
			case err := <-notifyClose:
				m.log.Error("closed rabbitmq/pubsub channel.. attempting to reconnect", slog.Any("err", err))
