...
func receiptProvider(status statusReporter) *facebookProvider {
...
func TestProcessReceipt_DeliveryWithMids(t *testing.T) {
	...
	p.processReceipt(context.Background(), &fbmodel.FacebookGate{ID: "gate-1"}, "psid-1", Messaging{
...
func TestProcessReceipt_DeliveryWatermarkOnly(t *testing.T) {
	...
	if status.deliveredUpTo != "psid-1" {
		t.Fatalf("expected watermark delivery for psid-1, got %q", status.deliveredUpTo)
	}
...
func TestProcessReceipt_ReadWatermark(t *testing.T) {
	...
	p.processReceipt(context.Background(), &fbmodel.FacebookGate{ID: "gate-1"}, "psid-9", Messaging{
		Read: &Read{Watermark: 1_770_000_222_000},
	})

	if status.readUpTo != "psid-9" {
		t.Fatalf("expected read watermark for psid-9, got %q", status.readUpTo)
	}
