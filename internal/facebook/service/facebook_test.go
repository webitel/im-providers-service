...
type mockFacebookStore struct {
	insertFn             func(ctx context.Context, dc int64, g *fbmodel.FacebookGate) error
	selectFn             func(ctx context.Context, id string) (*fbmodel.FacebookGate, error)
	selectByPageAndURIFn func(ctx context.Context, pageID, uri string) (*fbmodel.FacebookGate, error)
	updateFn             func(ctx context.Context, g *fbmodel.FacebookGate) error
...
var _ fbstore.FacebookStore = (*mockFacebookStore)(nil)

func stubFBGate() *fbmodel.FacebookGate {
...
func TestFacebookService_CreateGate_Success(t *testing.T) {
	var inserted *fbmodel.FacebookGate
	repo := &mockFacebookStore{
		insertFn: func(_ context.Context, dc int64, g *fbmodel.FacebookGate) error {
			if dc != 7 {
	...
	gate, err := svc.CreateGate(context.Background(), fbmodel.CreateFacebook{
		Name:      "Test Page",
		Dc:        7,
		MetaAppID: "app-1",
		PageID:    "page-1",
...
func TestFacebookService_CreateGate_StoreError(t *testing.T) {
	repo := &mockFacebookStore{
		insertFn: func(_ context.Context, _ int64, _ *fbmodel.FacebookGate) error {
			return errors.New("db error")
		},
	}
	svc := newFBService(repo)
	_, err := svc.CreateGate(context.Background(), fbmodel.CreateFacebook{Name: "X"})
	if err == nil {
...
func TestFacebookService_GetGate_Success(t *testing.T) {
	repo := &mockFacebookStore{
		selectFn: func(_ context.Context, id string) (*fbmodel.FacebookGate, error) {
			if id != "gate-1" {
...
func TestFacebookService_GetGate_NotFound(t *testing.T) {
	repo := &mockFacebookStore{
		selectFn: func(_ context.Context, _ string) (*fbmodel.FacebookGate, error) {
...
func TestFacebookService_UpdateGate_Name(t *testing.T) {
	repo := &mockFacebookStore{
		...
		updateFn: func(_ context.Context, g *fbmodel.FacebookGate) error {
			if g.Name != "New Name" {
	...
	svc := newFBService(repo)
	newName := "New Name"
	gate, err := svc.UpdateGate(context.Background(), fbmodel.UpdateFacebook{
		ID:   "gate-1",
		Name: &newName,
	})
	if err != nil {
...
func TestFacebookService_UpdateGate_Enabled(t *testing.T) {
	...
	svc := newFBService(repo)
	disabled := false
	_, err := svc.UpdateGate(context.Background(), fbmodel.UpdateFacebook{
		ID:      "gate-1",
		Enabled: &disabled,
	})
	if err != nil {
...
func TestFacebookService_UpdateGate_SelectError(t *testing.T) {
	...
	svc := newFBService(repo)
	name := "x"
	_, err := svc.UpdateGate(context.Background(), fbmodel.UpdateFacebook{ID: "missing", Name: &name})
	if !errors.Is(err, sharedstore.ErrNotFound) {
		t.Errorf("expected ErrNotFound, got %v", err)
	}
}
...
func TestFacebookService_UpdateGate_UpdateError(t *testing.T) {
	repo := &mockFacebookStore{
		...
		updateFn: func(_ context.Context, _ *fbmodel.FacebookGate) error {
			return errors.New("update failed")
		},
	}
	svc := newFBService(repo)
	name := "x"
	_, err := svc.UpdateGate(context.Background(), fbmodel.UpdateFacebook{ID: "gate-1", Name: &name})
	if err == nil {
		t.Fatal("expected error, got nil")
	}
}
